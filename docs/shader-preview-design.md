...
>
> **仍推荐的主路径**：`shader-render.py` 批量 PNG + `macode dev` / `macode render` 与 ShaderFrame。

> **原状态说明**：P3 / 设计完成，部分实现曾存在于 `bin/`，现已退役为 experimental
...
**核心原则**：
- ~~`report-generator.py`~~ **已删除（2026-05-17）**。原功能：生成幕画廊（帧 + issues + 代码片段）。当前检查报告以 JSON 形式输出到 `.agent/check_reports/*.json`，由 `macode check` 消费。
- 浏览器打开即可查看，无需服务器
- 每个 UI 元素可追溯到具体文件路径
...
> **状态（2026-05-17）**：`bin/dashboard-server.mjs` 已**彻底删除**（523 行，无人使用，维护成本高）。Dashboard 的所有功能由 `tail -f .agent/progress/*.jsonl` 替代。下文保留历史规格草图。

**原路径**：~~`bin/dashboard-server.mjs`~~
...
**Shader Preview 集成**（以下为原设计；**2026-05-13** 起 Dashboard 侧 Shader 分组已撤回，见 `CHANGELOG`）：
...
**组件（原计划；与当前仓库对照）**：
- `bin/macode-run`：统一进程生命周期 + 超时 + 信号转发（仍可用于其它引擎任务）
- ~~`server-guardian.mjs`~~：**MC 侧已移除**；TTL/回收若需重演由文档与 `macode mc stop` 人工兜底
- `bin/cleanup-stale.py`：将 dead PID 的 running 标记为 stalled，可选裁剪旧日志

**Shader Preview 集成（原计划，未完全实现）**：
- Preview 服务可由 `macode-run` 启动（若接线）；experimental 脚本当前为直接 `node`（见 `experimental/shader-preview/`）
- ~~Guardian `shader-preview` 类型扫描~~：未落地
- 空闲 TTL 后写入 `state.json` `stopped`：**依赖进程内实现**；experimental 仅 SIGINT/SIGTERM 关停示例
...
> **2026-05-13**：`macode shader preview` / `preview-stop` 已从稳定 CLI 移除；以下为**原计划**流程。实验性入口：`node experimental/shader-preview/shader-preview.mjs <asset_id>`（见 `experimental/shader-preview/README.md`）。

```
用户层命令（原计划；现已非 macode 路由）          内部实现
─────────────────────────────────────────────────────────────
node experimental/shader-preview/shader-preview.mjs <asset_id> [--port <n>]
    │                                   │
    │   ┌───────────────────────────────┘
    │   │
    │   ├── 1. signal-check.py --scene shader-preview-{asset}   （未接线）
    │   ├── 2. 读取 _registry.json → 定位素材目录
    │   ├── 3. 读取 shader.json → 提取 uniforms schema
    │   ├── 4. 预解析 frag.glsl（LYGIA #include 内联）          （未实现）
    │   ├── 5. GLSL 转译（#version 330 → WebGL 2 ES）
    │   ├── 6. macode-run 启动 HTTP preview 服务                 （未接线：直接 node）
    │   ├── 7. state-write.py → state.json                      （脚本自写 JSON）
    │   ├── 8. progress-write.py → jsonl                       （脚本自写）
    │   └── 9. 注册到 guardian                                  （未实现）
    │
    └── stdout: { "url": "...", "asset": "...", "port": ..., "pid": ... }

停止：向 pid 发送 SIGTERM，或 Ctrl+C（无 macode shader preview-stop）
```
...
**状态（2026-05-13）**：`bin/dashboard-server.mjs` 曾实现的 `listShaderPreviews()` 与侧栏「Shader Previews」分组已移除，避免与单 Agent 主路径耦合。若需桌面监控，可自行恢复该函数或单独 fork。

历史规格草图（仅供参考）：

```javascript
// ~~dashboard-server.mjs~~ 新增（已删除，历史规格仅供参考）
...
| `macode-run` | 直接启动 preview 服务（未接线） | 0 |
| ~~`dashboard-server.mjs`~~ | ~~曾新增 `listShaderPreviews()`~~ | **已删除** |
| ~~`report-generator.py`~~ | ~~新增 shader 专用报告模板~~ | **已删除** |
| `lygia_resolver.py` | Node.js 中移植简化版，或子进程调用 Python 预解析 | ~80 行 |
| `check-shader.py` | 新建：预渲染 PNG vs WebGL 输出对比 | ~100 行 |
| `experimental/shader-preview/shader-preview.mjs` | HTTP server + WebGL renderer + UI（非稳定） | ~500 行 |
...
```bash
# 实验性预览（非 macode 子命令；仓库根目录执行）
node experimental/shader-preview/shader-preview.mjs <asset_id> [--port <n>]

# 停止：根据 .agent/tmp/shader-preview-<asset_id>/state.json 的 pid kill，或对前台进程 Ctrl+C

# 以下为原计划中尚未存在的命令，保留作 backlog
# macode shader report <asset_id> [--output <dir>]
# bin/check-shader.py scenes/02_shader_mc/
```

**stdout JSON**（`shader-preview.mjs` 启动成功时）：
```json
{
...
| P1 | Dashboard 集成（listShaderPreviews + UI） | — | **已撤回**（2026-05-13） |
| P1 | Guardian 集成（shader-preview 类型） | ~20 | server-guardian |
| P1 | macode CLI 路由（shader preview / preview-stop） | — | **已撤回**（2026-05-13） |
| P2 | check-shader.py（对比检查） | ~100 | 无 |
| P2 | ~~报告生成器扩展（shader 专用报告）~~ | — | **已删除** |
...
| 检查 | `.agent/check_reports/*.json` | check-runner.py | macode check / tail | JSON |
| 报告 | ~~`.agent/reports/*/index.html`~~ | ~~report-generator.py~~ | ~~人类浏览器~~ | **已删除** |
