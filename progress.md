...
## v0.6.0：交互式开发循环与可视化规划（2026-05-08）

**目标**：建立幕级工作流——注释 → 提取 → 检查 → 报告 → 开发 → 人类介入。Agent 修改幕，机器自动验证，人类审核报告。

### 核心概念：幕（Act）

幕是场景内的逻辑分段，三层存储：
1. **注释层**：代码中 `# @act:intro`、`# @time:0-1.1s`、`# @keyframes:[0, 0.8, 1.1]`
2. **缓存层**：`scenes/{name}/acts.json`（`extract-acts.py` 提取）
3. **权威层**：`manifest.json` 的 `acts` 字段（`--sync` 同步）

### 新增工具

| 工具 | 职责 | 输出 |
|------|------|------|
| `bin/extract-acts.py` | 从源码提取 `@act:` 注释 | `acts.json` + 同步 `manifest.json` |
| `bin/check-static.py` | 幕级静态检查（时长一致性、公式密度、注释同步） | `.agent/check_reports/{scene}_static.json` |
| `bin/check-frames.py` | 按 keyframes 采样帧，Pillow CV 分析 | `.agent/check_reports/{scene}_frames.json` + 红框标注 PNG |
| `bin/report-generator.py` | 生成 HTML 幕画廊报告 | `.agent/reports/{scene}/index.html` |
| `bin/timeline-generator.py` | 生成项目级 HTML 甘特图 | `.agent/reports/timeline.html` |
| `bin/macode-dev.sh` | 开发模式入口（按引擎分发） | `.agent/tmp/{scene}/preview.mp4` |
| `bin/signal-check.sh` | 检查人类介入信号 | JSON stdout |
| `engines/manim/scripts/dev.sh` | ManimCE 低分辨率快速预览 | `preview.mp4` |

### CLI 集成

```bash
macode check <scene_dir> [--static|--frames]   # 运行检查，输出 JSON
macode report <scene_dir> [--open]             # 生成 HTML 幕画廊
macode timeline [--open]                       # 生成项目时间轴
macode dev <scene_dir> [--act <act_id>]        # 开发模式
```

### 人类介入机制

`.agent/signals/` 目录作为纯文件系统信号：
- `pause` — 暂停迭代
- `abort` — 终止运行
- `review_needed` — 审核门
- `reject` — 拒绝方案
- `human_override.json` — 决策覆盖

无守护进程，Host Agent 在每次迭代前调用 `signal-check.sh` 读取状态。

### 已知限制

- **overlap 检测误报**：基于 Canny 边缘 + IOU，可能对单个元素的内部边缘产生误报（如标题多笔画）。后续可优化为基于颜色/纹理的区域分割。
- **check-frames 依赖预渲染帧**：需要场景已渲染（`raw.mp4` 或 `frames/` 存在），否则尝试用 ffmpeg 截取。
- **dev.sh manim 模式是全量预览**：修改 manifest 做低质量渲染，非真正的幕级渲染。幕级优化由 `check --frames` 实现。

### 初心一致性

| 初心 | 评估 |
|------|------|
| **Bash-First** | ✅ 所有工具都是独立 CLI |
| **管道透明** | ✅ JSON/HTML 标准格式，浏览器消费 |
| **Agent 可理解** | ✅ `@act:` 注释即文档 |
| **UNIX 哲学** | ✅ 每个工具单一职责 |
| **Host Agent 不竞争** | ✅ 无守护进程，无自定义 shell |

---

*路线图版本：v0.6*  
*哲学：Make it work → Make it right → Make it fast*  
*当前阶段：v0.6.0 — 交互式开发循环与可视化规划 —— 2026-05-08*
