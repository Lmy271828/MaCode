#!/usr/bin/env python3
"""bin/extract-acts.py
从场景源码中提取 @act: 注释，输出 acts.json。

用法:
    extract-acts.py scenes/02_fourier/scene.py > scenes/02_fourier/acts.json
    extract-acts.py --sync scenes/02_fourier/  # 同步到 manifest.json
"""

import re
import json
import sys
import os

COMMENT_PREFIX = {
    '.py': '#',
    '.ts': '//',
    '.tsx': '//',
    '.js': '//',
}

def extract(filepath):
    ext = os.path.splitext(filepath)[1]
    prefix = COMMENT_PREFIX.get(ext, '#')
    
    p = re.escape(prefix)
    ACT_RE = re.compile(r'^\s*' + p + r'\s*@act:(\w+)\s*$')
    TIME_RE = re.compile(r'^\s*' + p + r'\s*@time:([\d.]+)-([\d.]+)s\s*$')
    KEYFRAMES_RE = re.compile(r'^\s*' + p + r'\s*@keyframes:\[(.*?)\]\s*$')
    DESC_RE = re.compile(r'^\s*' + p + r'\s*@description:(.*)$')
    CHECKS_RE = re.compile(r'^\s*' + p + r'\s*@checks:\[(.*?)\]\s*$')
    
    acts = []
    current = None
    
    with open(filepath, 'r', encoding='utf-8') as f:
        for lineno, line in enumerate(f, 1):
            m = ACT_RE.match(line)
            if m:
                if current:
                    current['line_end'] = lineno - 1
                current = {
                    'id': m.group(1),
                    'file': os.path.basename(filepath),
                    'line_start': lineno,
                }
                acts.append(current)
                continue
            
            if not current:
                continue
            
            m = TIME_RE.match(line)
            if m:
                current['time_range'] = [float(m.group(1)), float(m.group(2))]
                continue
            
            m = KEYFRAMES_RE.match(line)
            if m:
                current['keyframes'] = [
                    float(x.strip()) for x in m.group(1).split(',') if x.strip()
                ]
                continue
            
            m = DESC_RE.match(line)
            if m:
                current['description'] = m.group(1).strip()
                continue
            
            m = CHECKS_RE.match(line)
            if m:
                current['checks'] = [
                    x.strip().strip('"\'') for x in m.group(1).split(',') if x.strip()
                ]
                continue
            
            if line.strip() and not line.strip().startswith(prefix):
                current['line_end'] = lineno - 1
                current = None
    
    if current and 'line_end' not in current:
        with open(filepath, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            current['line_end'] = len(lines)
    
    return {'acts': acts, 'source_file': os.path.basename(filepath)}

def sync_to_manifest(acts_data, manifest_path):
    if not os.path.exists(manifest_path):
        print(f"Error: manifest not found: {manifest_path}", file=sys.stderr)
        sys.exit(1)
    
    with open(manifest_path, 'r', encoding='utf-8') as f:
        manifest = json.load(f)
    
    manifest['acts'] = acts_data['acts']
    
    with open(manifest_path, 'w', encoding='utf-8') as f:
        json.dump(manifest, f, indent=2, ensure_ascii=False)
        f.write('\n')
    
    print(f"Synced {len(acts_data['acts'])} acts to {manifest_path}")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: extract-acts.py <scene.py> [--sync]", file=sys.stderr)
        sys.exit(1)
    
    filepath = sys.argv[1]
    result = extract(filepath)
    
    if '--sync' in sys.argv:
        scene_dir = os.path.dirname(os.path.abspath(filepath))
        manifest_path = os.path.join(scene_dir, 'manifest.json')
        sync_to_manifest(result, manifest_path)
    else:
        print(json.dumps(result, indent=2, ensure_ascii=False))
