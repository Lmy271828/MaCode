...
def collect_scenes():
    """遍历所有场景，收集 manifest 中的 segments 和 duration。"""
    scenes = []
...
""")
        if segments:
            for idx, seg in enumerate(segments):
                ...
                color = act_color(idx)
                sid = seg.get("id", f"seg{idx}")
                desc = seg.get("description", "")
                tooltip = f"{sid}: {t0}-{t1}s"
                if desc:
                    tooltip += f" — {desc}"
...
""")
    ...
    html_parts.append("""  </div>
  <div class="legend">
    <p>Each row represents a scene. Colored segments. Hover for details.</p>
  </div>
</body>
</html>
""")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate project-level timeline HTML (Gantt-style).',
        epilog='Output: .agent/reports/timeline.html',
