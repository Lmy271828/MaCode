...
def load_segments(scene_dir):
    """从 manifest.json 或 segments.json 读取 segments。"""
    manifest_path = os.path.join(scene_dir, "manifest.json")
    segments_path = os.path.join(scene_dir, "segments.json")
...
def build_issue_map(reports):
    ...
    for report in reports:
        for seg in report.get("segments", []):
            sid = seg["id"]
            if sid not in act_issues:
                act_issues[sid] = {"status": seg.get("status", "pass"), "issues": []}
            act_issues[sid]["issues"].extend(seg.get("issues", []))
            # 提升状态优先级: error > warning > pass
            status_order = {"error": 3, "warning": 2, "pass": 1}
            old = act_issues[sid]["status"]
            new = seg.get("status", "pass")
            if status_order.get(new, 0) > status_order.get(old, 0):
                act_issues[sid]["status"] = new
    return act_issues
...
def generate_html(scene_dir):
    ...
    segments, manifest = load_segments(scene_dir)
    if not segments:
        print(f"No segments found for {scene_name}", file=sys.stderr)
        sys.exit(1)

    duration = manifest.get("duration", 0)
    if not duration and segments:
        duration = segments[-1].get("time_range", [0, 0])[-1]
...
<body>
  ...
  <p class="meta">Total segments: {len(segments)} | Duration: {duration}s | Engine: {escape_html(manifest.get('engine', 'unknown'))}</p>
""")

    # 尝试读取源码以便展示代码片段
    source_file = segments[0].get("file", "scene.py") if segments else "scene.py"
    ...
    for seg in segments:
        ...
        keyframes = seg.get("keyframes", [t_start, t_end])
        description = seg.get("description", "")
        line_start = seg.get("line_start", 0)
        line_end = seg.get("line_end", 0)

        info = issue_map.get(sid, {})
    ...
    <div class="act-header">
      <h3>{escape_html(sid)} ({t_start}-{t_end}s)</h3>
...
""")
        ...
        for t in keyframes:
            fpath = find_frame_path(scene_name, sid, t, fps)
            st = frame_status(sid, t)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate scene-level HTML report (segment gallery with frames and issues).',
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument('scene_dir', help='Path to scene directory')
    args = parser.parse_args()
    generate_html(args.scene_dir)
