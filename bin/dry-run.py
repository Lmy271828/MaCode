...
def check_latex(path: str, engine: str) -> tuple[bool, int, int, list[tuple[str, int, str, str]]]:
    ...
    if formulas:
        ...
        for (fpath, line, expr), obj in zip(formulas, results):
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: dry-run.py <scene_file> [engine]", file=sys.stderr)
        return 2

    scene_file = sys.argv[1]
    engine = sys.argv[2] if len(sys.argv) > 2 else "manim"
