#!/usr/bin/env node
/**
 * bin/shader-preview.mjs
 * MaCode Shader Preview — Human monitoring extension for Layer 2 shader assets.
 *
 * Design (Harness 2.0):
 ...
 *
 * Usage:
 *   shader-preview.mjs <asset_id> [--port <n>]
 */
...
if (!assetId || assetId === '--help' || assetId === '-h') {
  console.log(`Usage: node shader-preview.mjs <asset_id> [--port <n>]

Start a WebGL 2 shader preview server for a Layer 2 asset.
...
Examples:
  node shader-preview.mjs lygia_circle_heatmap
  node shader-preview.mjs lygia_circle_heatmap --port 8765`);
  process.exit(0);
}
