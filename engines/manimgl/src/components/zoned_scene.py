...
ZoneScene — Declarative spatial constraint system for ManimGL.

Orchestration layer: loads layout → validates constraints → computes
geometry → executes placement.  Pure geometry and validation live in
``utils.layout_geometry`` and ``utils.layout_validator``.
...
class ZoneScene(Scene):
    ...
    from ``engines/manimgl/src/templates/layouts/``.
    """
    ...
    def place(
        ...
        Args:
            ...
            zone_name: Key in the layout's ``zones`` dictionary.
            align: One of ``center`` (default), ``top``, ``bottom``,
                ``left``, ``right``.
            offset: Additional shift applied *after* zone alignment.

        Returns:
            The same *mobj* (for chaining).

        Raises:
            ZoneNotFoundError: If *zone_name* is unknown.
            ZoneOverflowError: If the zone is full.
            ZoneTypeError: If *mobj*'s type is not in ``allowed_types``.
        """
    ...
    def zone_bounds(self, zone_name: str) -> dict[str, float]:
        ...
        Keys: ``left``, ``right``, ``top``, ``bottom`` (Manim coords).
        """
        if zone_name not in self._zones:
            raise ZoneNotFoundError(zone_name)
