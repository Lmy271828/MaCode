...
#
# 用法:
#   audio-analyze.sh <audio_file> <output_csv> [fps=30]
#
# 输出 CSV 格式:
#   time,beat,loudness,low,mid,high
#   time   - 秒数（与视频帧对齐）
#   beat   - 1=检测到节拍（低频峰值），0=无
#   loudness - 整体响度（0.0~1.0 归一化）
#   low    - 低频能量（<100Hz，鼓点）
#   mid    - 中频能量（100Hz~2kHz，人声/乐器）
#   high   - 高频能量（>2kHz，镲片/高频）
